...
export function findOptimalHalls(requiredStrength: number, availableRooms: Room[]): string[] {
  ...
  // Try to find exact match or best fitting combination using greedy search
  ...
  // If still not enough, select all active rooms
  if (currentCapacity < requiredStrength) {
...
export function allocateSeats(
  ...
  // Prepare students based on strategy
  ...
  if (strategy === 'interleaved-dept') {
    ...
    // Sequential by department then roll no
  ...
  for (const room of activeRooms) {
    ...
    // Calculate max rows across all columns for grid transversal
    const maxRows = Math.max(
      room.rows,
      ...(room.columnRows && room.columnRows.length > 0 ? room.columnRows : [room.rows])
    );

    // Traverse row-by-row across columns for balanced exam hall distribution
        ...
        for (let sIdx = 0; sIdx < colPerDesk; sIdx++) {
          if (studentIndex < orderedStudents.length) {
            ...
            const slotPos = colPerDesk === 1 
              ? 'Single' 
              : String.fromCharCode(65 + sIdx); // 'A', 'B', 'C', 'D', etc.
...
export function autoAssignFacultyDuties(
  ...
  // 1. Assign Chief Superintendent (Senior Professor or Assoc Prof)
  ...
  // 2. Assign Hall Invigilators for each active room
  usedRooms.forEach((room) => {
    // Select faculty with lowest duties count
    let assigned = false;
  ...
  // 3. Assign 1-2 Standby Relievers
