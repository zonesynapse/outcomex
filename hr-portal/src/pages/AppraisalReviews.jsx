...
export default function AppraisalReviews() {
  ...
  const renderRowEvidenceHeader = (sectionId) => {
    ...
    return <th className="border border-slate-200 p-2 text-indigo-600enter w-28 text-[10px] uppercase font-bold text-slate-700 bg-slate-50">Evidence</th>;
  };

  const renderRowEvidenceCellReadOnly = (row, sectionId) => {
    if (!isSectionEvidenceRequired(sectionId)) return null;
    return (
      <td className="border border-slate-200 p-1.5 text-indigo-600enter min-w-[100px]">
        {row.fileUrl ? (
          <a 
            href={row.fileUrl} 
            target="_blank" 
  ...
  const renderReviewCustomFields = (tId, customFieldsData) => {
    ...
    return (
      <div className="mt-6 pt-6 border-t border-slate-200/60 space-y-4 text-xs">
        <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-1.5 mb-2">
          ...
          {tabFields.map((field) => {
            ...
            return (
              <div key={field.id} className="bg-slate-50 border border-slate-200/50 p-4 rounded-xl space-y-2">
                <span className="block text-[9px] font-black text-indigo-400 uppercase tracking-wider">{field.title}</span>
                {field.description && (
                  <p className="text-[9px] text-slate-600 font-semibold uppercase leading-tight">{field.description}</p>
                )}
                {field.type !== "file_only" && savedEntry.value && (
                  <p className="font-bold text-slate-900 glass-input p-3 rounded-lg text-slate-900">{savedEntry.value}</p>
                )}
                {field.evidenceRequired && savedEntry.fileUrl && (
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-slate-600 font-semibold uppercase text-[9px]">Proof Attachment:</span>
                    <a 
...
// Fetch all appraisal records (Teaching, Non-Teaching, HOD)
  ...
  const filteredAppraisals = appraisals.filter((app) => {
    ...
    // Strict School Isolation: Only display data for logged-in user's school
    const userInst = userProfile?.institution;
    const appInst = app.institution || app.formData?.institution || app.school;
    const instMatch = !userInst || !appInst || userInst === appInst;

    const depMatch = (userRole === "HOD" || userRole === "Coordinator")
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {selectedAppraisal ? (
          <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden animate-fadeIn">
            ...
            <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex flex-wrap items-center justify-between gap-4">
              <button
                ...
                className="px-3.5 py-1.5 border border-slate-200 text-slate-700 hover:bg-zinc-100 rounded-xl text-xs font-bold transition-all flex items-center gap-1 cursor-pointer"
              >
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                  ...
                  selectedAppraisal.status === "Submitted" ? "bg-indigo-600mber-500/20 text-indigo-600mber-700 border border-amber-500/30" :
                  selectedAppraisal.status === "Returned" ? "bg-rose-500/20 text-rose-700 border border-rose-500/30" :
                  "bg-slate-500/20 text-slate-800 border border-zinc-500/30"
                }`}>
            ...
            <div className="p-6 md:p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="flex border-b border-slate-200 overflow-x-auto gap-2 no-scrollbar mb-4">
                  ...
                  <div className="space-y-6">
                    {isSectionVisible("sec_profile_details") && (
                      <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 space-y-3">
                        <span style={{ fontSize: "11px" }} className="font-extrabold text-indigo-950 block border-b border-slate-200 pb-1 uppercase tracking-wider">
                          {getSectionTitle("sec_profile_details", "1.1 Profile Details")}
                        </span>
                        {getSectionDescription("sec_profile_details") && (
                          <p className="text-[10px] text-slate-600 font-semibold uppercase">{getSectionDescription("sec_profile_details")}</p>
                        )}
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-xs">
                          {isSectionVisible("f_name") && (
                            <div>
                              <span className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider">{getSectionTitle("f_name", "Faculty Name")}</span>
                              <span className="font-bold text-slate-900">{selectedAppraisal.formData?.name || selectedAppraisal.facultyName}</span>
                            </div>
                          )}
                          {isSectionVisible("f_designation") && (
                            <div>
                              <span className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider">{getSectionTitle("f_designation", "Designation")}</span>
                              <span className="font-bold text-slate-900">{selectedAppraisal.formData?.designation || selectedAppraisal.designation}</span>
                            </div>
                          )}
                          {isSectionVisible("f_department") && (
                            <div>
                              <span className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider">{getSectionTitle("f_department", "Department")}</span>
                              <span className="font-bold text-slate-900">{selectedAppraisal.formData?.department || selectedAppraisal.department}</span>
                            </div>
                          )}
                          {isSectionVisible("f_dob") && (
                            <div>
                              <span className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider">{getSectionTitle("f_dob", "Date of Birth")}</span>
                              <span className="font-bold text-slate-900">{selectedAppraisal.formData?.dob || "-"}</span>
                            </div>
                          )}
                          {isSectionVisible("f_age") && (
                            <div>
                              <span className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider">{getSectionTitle("f_age", "Age")}</span>
                              <span className="font-bold text-slate-900">{selectedAppraisal.formData?.age || "-"}</span>
                            </div>
                          )}
                          {isSectionVisible("f_subjectSpecialization") && (
                            <div>
                              <span className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider">{getSectionTitle("f_subjectSpecialization", "Specialization / Interest")}</span>
                              <span className="font-bold text-slate-900">{selectedAppraisal.formData?.subjectSpecialization || "-"}</span>
                            </div>
                          )}
                          {isSectionVisible("f_dojCollege") && (
                            <div>
                              <span className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider">{getSectionTitle("f_dojCollege", "DOJ College")}</span>
                              <span className="font-bold text-slate-900">{selectedAppraisal.formData?.dojCollege || "-"}</span>
                            </div>
                          )}
                          {isSectionVisible("f_dojPresentPost") && (
                            <div>
                              <span className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider">{getSectionTitle("f_dojPresentPost", "DOJ Present Post")}</span>
                              <span className="font-bold text-slate-900">{selectedAppraisal.formData?.dojPresentPost || "-"}</span>
                            </div>
                          )}
                          {isSectionVisible("f_academicQualification") && (
                            <div>
                              <span className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider">{getSectionTitle("f_academicQualification", "Academic Qualification")}</span>
                              <span className="font-bold text-slate-900">{selectedAppraisal.formData?.academicQualification || "-"}</span>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    {isSectionVisible("sec_profile_experience") && (
                      <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 space-y-3">
                        ...
                        {getSectionDescription("sec_profile_experience") && (
                          <p className="text-[10px] text-slate-600 font-semibold uppercase">{getSectionDescription("sec_profile_experience")}</p>
                        )}
                        <div className="grid grid-cols-3 gap-4">
                          {isSectionVisible("f_teachingCKCET") && (
                            <div className="bg-white border border-slate-200 p-3 rounded-xl text-indigo-600enter">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider truncate">{getSectionTitle("f_teachingCKCET", "Teaching CKCET")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.experience?.teachingCKCET || "0"} Yrs</span>
                            </div>
                          )}
                          {isSectionVisible("f_teachingElsewhere") && (
                            <div className="bg-white border border-slate-200 p-3 rounded-xl text-indigo-600enter">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider truncate">{getSectionTitle("f_teachingElsewhere", "Teaching Elsewhere")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.experience?.teachingElsewhere || "0"} Yrs</span>
                            </div>
                          )}
                          {isSectionVisible("f_industrial") && (
                            <div className="bg-white border border-slate-200 p-3 rounded-xl text-indigo-600enter">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider truncate">{getSectionTitle("f_industrial", "Industrial")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.experience?.industrial || "0"} Yrs</span>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    {isSectionVisible("sec_profile_workload") && (
                      <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 space-y-3">
                        ...
                        {getSectionDescription("sec_profile_workload") && (
                          <p className="text-[10px] text-slate-600 font-semibold uppercase">{getSectionDescription("sec_profile_workload")}</p>
                        )}
                        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                          {isSectionVisible("f_oddTheory") && (
                            <div className="bg-white border border-slate-200 p-3 rounded-xl text-indigo-600enter">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider truncate">{getSectionTitle("f_oddTheory", "Odd Theory")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.workloadWeek?.oddTheory || "0"} Hrs</span>
                            </div>
                          )}
                          {isSectionVisible("f_oddPractical") && (
                            <div className="bg-white border border-slate-200 p-3 rounded-xl text-indigo-600enter">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider truncate">{getSectionTitle("f_oddPractical", "Odd Lab")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.workloadWeek?.oddPractical || "0"} Hrs</span>
                            </div>
                          )}
                          {isSectionVisible("f_oddTotal") && (
                            <div className="bg-white border border-slate-200 p-3 rounded-xl text-indigo-600enter">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider truncate">{getSectionTitle("f_oddTotal", "Odd Total")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.workloadWeek?.oddTotal || "0"} Hrs</span>
                            </div>
                          )}
                          {isSectionVisible("f_evenTheory") && (
                            <div className="bg-white border border-slate-200 p-3 rounded-xl text-indigo-600enter">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider truncate">{getSectionTitle("f_evenTheory", "Even Theory")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.workloadWeek?.evenTheory || "0"} Hrs</span>
                            </div>
                          )}
                          {isSectionVisible("f_evenPractical") && (
                            <div className="bg-white border border-slate-200 p-3 rounded-xl text-indigo-600enter">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider truncate">{getSectionTitle("f_evenPractical", "Even Lab")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.workloadWeek?.evenPractical || "0"} Hrs</span>
                            </div>
                          )}
                          {isSectionVisible("f_evenTotal") && (
                            <div className="bg-white border border-slate-200 p-3 rounded-xl text-indigo-600enter">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider truncate">{getSectionTitle("f_evenTotal", "Even Total")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.workloadWeek?.evenTotal || "0"} Hrs</span>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <>
                        ...
                        <div className="border-b border-slate-100 pb-2 mb-4">
                          ...
                          {getSectionDescription("sec_subjects_results") && (
                            <p className="text-[10px] text-slate-600 font-semibold uppercase mt-0.5">{getSectionDescription("sec_subjects_results")}</p>
                          )}
                        </div>

                        {/* Odd Semester */}
                        <div>
                          <span className="text-xs font-black text-indigo-400 block mb-2 uppercase tracking-wider">Odd Semester Theory Subjects</span>
                          <table className="w-full border-collapse border border-slate-200 text-xs">
                            <tr className="bg-slate-50 font-bold">
                              ...
                              <th className="border border-slate-200 p-2 text-indigo-600enter">Appeared</th>
                              <th className="border border-slate-200 p-2 text-indigo-600enter">Passed</th>
                              <th className="border border-slate-200 p-2 text-indigo-600enter">% Result</th>
                              <th className="border border-slate-200 p-2 text-indigo-600enter">Feedback Rating</th>
                              {renderRowEvidenceHeader("sec_subjects_results")}
                            </tr>
                            {(selectedAppraisal.formData?.oddTheorySubjects || []).map((row, i) => (
                              <tr key={i}>
                                <td className="border border-slate-200 p-2">{row.class}</td>
                                <td className="border border-slate-200 p-2">{row.subjectCodeTitle}</td>
                                <td className="border border-slate-200 p-2 text-indigo-600enter">{row.appeared}</td>
                                <td className="border border-slate-200 p-2 text-indigo-600enter">{row.passed}</td>
                                <td className="border border-slate-200 p-2 text-indigo-600enter font-bold text-indigo-400">{row.resultPercentage}</td>
                                <td className="border border-slate-200 p-2 text-indigo-600enter font-bold text-indigo-700">{row.feedbackRating}</td>
                        ...
                        <div>
                          <span className="text-xs font-black text-indigo-400 block mb-2 uppercase tracking-wider">Odd Semester Practical / Project Subjects</span>
                          <table className="w-full border-collapse border border-slate-200 text-xs">
                            <tr className="bg-slate-50 font-bold">
                              <th className="border border-slate-200 p-2">Class</th>
                              <th className="border border-slate-200 p-2">Subject Code & Title</th>
                        ...
                        {/* Even Semester */}
                        <div>
                          <span className="text-xs font-black text-indigo-400 block mb-2 uppercase tracking-wider">Even Semester Theory Subjects</span>
                          <table className="w-full border-collapse border border-slate-200 text-xs">
                            <tr className="bg-slate-50 font-bold">
                              <th className="border border-slate-200 p-2">Class</th>
                              <th className="border border-slate-200 p-2">Subject Code & Title</th>
                        ...
                        <div>
                          <span className="text-xs font-black text-indigo-400 block mb-2 uppercase tracking-wider">Even Semester Practical / Project Subjects</span>
                          <table className="w-full border-collapse border border-slate-200 text-xs">
                            <tr className="bg-slate-50 font-bold">
                              <th className="border border-slate-200 p-2">Class</th>
                              <th className="border border-slate-200 p-2">Subject Code & Title</th>
                        ...
                        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                          <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider">11. Result Attribution Opinion</span>
                          <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.resultAttribution || "Both"}</span>
                        </div>
                      </>
                    )}
                ...
                {activeDetailsTab === 3 && (
                  <div className="space-y-6">
                    {isSectionVisible("sec_academic_nptel") && (
                      <div>
                        <span className="text-xs font-black text-indigo-400 block mb-2 uppercase tracking-wider">
                        ...
                        <table className="w-full border-collapse border border-slate-200 text-xs">
                          <tr className="bg-slate-50 font-bold">
                            <th className="border border-slate-200 p-2">Course Title</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">Start Date</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">End Date</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">Platform</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">Exam Date</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">Cert?</th>
                            {renderRowEvidenceHeader("sec_academic_nptel")}
                          </tr>
                          {(selectedAppraisal.formData?.onlineCourses || []).map((row, i) => (
                            <tr key={i}>
                              <td className="border border-slate-200 p-2 font-semibold">{row.title}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter">{row.startDate}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter">{row.endDate}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter">{row.platform}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter">{row.examDate}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter font-bold">{row.certificateReceived}</td>
                              {renderRowEvidenceCellReadOnly(row, "sec_academic_nptel")}
                            </tr>
                          ))}
                        </table>
                        {selectedAppraisal.formData?.onlineCoursesOutcome && (
                          <div className="bg-slate-50 border border-slate-200 p-3 rounded-xl mt-2 text-xs">
                            <span className="font-bold block mb-1">Outcome/Achievements of Courses:</span>
                            <p className="text-slate-700 font-medium">"{selectedAppraisal.formData.onlineCoursesOutcome}"</p>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {isSectionVisible("sec_academic_fdp") && (
                      <div>
                        ...
                        <table className="w-full border-collapse border border-slate-200 text-xs">
                          <tr className="bg-slate-50 font-bold">
                            <th className="border border-slate-200 p-2 text-left">Program Title</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">Dates</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">Days</th>
                            <th className="border border-slate-200 p-2 text-left">Organizing Institution</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">Report?</th>
                            {renderRowEvidenceHeader("sec_academic_fdp")}
                          </tr>
                          {(selectedAppraisal.formData?.workshopsFDPs || []).map((row, i) => (
                            <tr key={i}>
                              <td className="border border-slate-200 p-2 font-semibold text-slate-700">{row.title}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter">{row.dates}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter">{row.days}</td>
                              <td className="border border-slate-200 p-2 font-medium text-zinc-650">{row.organization}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter font-bold text-emerald-700">{row.reportSubmitted}</td>
                    ...
                    {isSectionVisible("sec_academic_journals") && (
                      <div>
                        ...
                        <table className="w-full border-collapse border border-slate-200 text-xs">
                          <tr className="bg-slate-50 font-bold">
                            <th className="border border-slate-200 p-2">Paper Title</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">Date/Month/Year</th>
                            <th className="border border-slate-200 p-2">Journal/Conference Name</th>
                            <th className="border border-slate-200 p-2">Volume & Page Details</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">SCI/SCOPUS/UGC</th>
                            {renderRowEvidenceHeader("sec_academic_journals")}
                          </tr>
                          {(selectedAppraisal.formData?.researchPapers || []).map((row, i) => (
                            <tr key={i}>
                              <td className="border border-slate-200 p-2 font-semibold">{row.title}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter">{row.dateMonthYear}</td>
                              <td className="border border-slate-200 p-2">{row.journal}</td>
                              <td className="border border-slate-200 p-2">{row.volumeIssue}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter font-bold text-indigo-700">{row.sciScopusUgc}</td>
                    ...
                    {isSectionVisible("sec_academic_nptel") && selectedAppraisal.formData?.improvingQualification && (
                      <div className="bg-[#120c7a]/5 border border-[#120c7a]/15 p-4 rounded-xl">
                        <span className="block text-[11px] font-bold text-slate-700 uppercase mb-2">Improving Qualification detail</span>
                        <table className="w-full border-collapse border border-slate-200 text-xs bg-white">
                          <tr className="bg-slate-50 font-bold">
                            <th className="border border-slate-200 p-1.5">Degree</th>
                            ...
                            <th className="border border-slate-200 p-1.5 text-indigo-600enter">NOC Obtained?</th>
                            {renderRowEvidenceHeader("sec_academic_nptel")}
                          </tr>
                          {(selectedAppraisal.formData?.improvingDetails || []).map((row, i) => (
                            <tr key={i}>
                              <td className="border border-slate-200 p-1.5">{row.degreeRegistered}</td>
                              <td className="border border-slate-200 p-1.5">{row.specialization}</td>
                              <td className="border border-slate-200 p-1.5">{row.university}</td>
                              <td className="border border-slate-200 p-1.5 text-indigo-600enter">{row.duration}</td>
                              <td className="border border-slate-200 p-1.5 text-indigo-600enter font-bold">{row.status}</td>
                              <td className="border border-slate-200 p-1.5 text-indigo-600enter">{row.nocObtained}</td>
                              {renderRowEvidenceCellReadOnly(row, "sec_academic_nptel")}
                            </tr>
                          ))}
                        </table>
                      ...
                      <>
                        <div className="border-b border-slate-100 pb-2 mb-4">
                          ...
                          {getSectionDescription("sec_roles_department") && (
                            <p className="text-[10px] text-slate-600 font-semibold uppercase mt-0.5">{getSectionDescription("sec_roles_department")}</p>
                          )}
                        </div>

                        {/* Organizing Programs */}
                        <div>
                          <span className="text-xs font-black text-slate-900 block mb-2 uppercase">A) Organizing FDP / Conferences / Workshops / Guest Lectures</span>
                          <table className="w-full border-collapse border border-slate-200 text-xs">
                            <tr className="bg-slate-50 font-bold">
                              <th className="border border-slate-200 p-2 text-left">Event Title</th>
                              <th className="border border-slate-200 p-2 text-indigo-600enter">Period</th>
                              <th className="border border-slate-200 p-2 text-left">Resource Details</th>
                              <th className="border border-slate-200 p-2 text-left">Target Audience & Outcome</th>
                              {renderRowEvidenceHeader("sec_roles_department")}
                            </tr>
                            {(selectedAppraisal.formData?.organizingPrograms || []).map((row, i) => (
                              <tr key={i}>
                                <td className="border border-slate-200 p-2 font-semibold text-slate-700">{row.title}</td>
                                <td className="border border-slate-200 p-2 text-indigo-600enter">{row.period}</td>
                                <td className="border border-slate-200 p-2 font-medium text-zinc-655">{row.resourcePersonDetails}</td>
                                <td className="border border-slate-200 p-2 text-slate-700">{row.targetAudience} - {row.outcome}</td>
                        ...
                        {/* Funding Proposals */}
                        <div>
                          <span className="text-xs font-black text-slate-900 block mb-2 uppercase">B) Contribution towards Funding Proposals / Testing / Consultancy</span>
                          <table className="w-full border-collapse border border-slate-200 text-xs">
                            <tr className="bg-slate-50 font-bold">
                              <th className="border border-slate-200 p-2 text-left">Proposal / Project Title</th>
                              <th className="border border-slate-200 p-2 text-indigo-600enter">Role</th>
                              <th className="border border-slate-200 p-2 text-indigo-600enter">Fund Requested (Rs)</th>
                              <th className="border border-slate-200 p-2 text-indigo-600enter">Agency & Status</th>
                              {renderRowEvidenceHeader("sec_roles_department")}
                            </tr>
                            {(selectedAppraisal.formData?.fundingProposals || []).map((row, i) => (
                              <tr key={i}>
                                <td className="border border-slate-200 p-2 font-semibold text-slate-700">{row.title}</td>
                                <td className="border border-slate-200 p-2 text-indigo-600enter font-bold text-indigo-700">{row.role}</td>
                                <td className="border border-slate-200 p-2 text-indigo-600enter font-bold text-emerald-700">{row.fundRequested}</td>
                                <td className="border border-slate-200 p-2 text-indigo-600enter font-medium">{row.fundingAgencyScheme} ({row.status})</td>
                                {renderRowEvidenceCellReadOnly(row, "sec_roles_department")}
                              </tr>
                            ))}
                          </table>
                        ...
                        {selectedAppraisal.formData?.involvementPlacement && selectedAppraisal.formData.involvementPlacement.length > 0 && (
                          <div>
                            <span className="text-xs font-black text-slate-900 block mb-2 uppercase">C) Placement Activities / Mentoring / Counseling</span>
                            <table className="w-full border-collapse border border-slate-200 text-xs">
                              <tr className="bg-slate-50 font-bold">
                                <th className="border border-slate-200 p-2 text-left">Description</th>
                                <th className="border border-slate-200 p-2">Role</th>
                                <th className="border border-slate-200 p-2">Outcome</th>
                                <th className="border border-slate-200 p-2 text-indigo-600enter">Records?</th>
                                {renderRowEvidenceHeader("sec_roles_department")}
                              </tr>
                              {selectedAppraisal.formData.involvementPlacement.map((row, i) => (
                                <tr key={i}>
                                  <td className="border border-slate-200 p-2 font-semibold text-slate-700">{row.description}</td>
                                  <td className="border border-slate-200 p-2 font-bold text-indigo-700">{row.role}</td>
                                  <td className="border border-slate-200 p-2 text-slate-700">{row.outcome}</td>
                                  <td className="border border-slate-200 p-2 text-indigo-600enter font-bold">{row.recordsMaintained}</td>
                                  {renderRowEvidenceCellReadOnly(row, "sec_roles_department")}
                                </tr>
                              ))}
                            </table>
                        ...
                        {selectedAppraisal.formData?.accreditationContributions && selectedAppraisal.formData.accreditationContributions.length > 0 && (
                          <div>
                            <span className="text-xs font-black text-slate-900 block mb-2 uppercase">D) ISO / NAAC / NBA / Coordinator role</span>
                            <table className="w-full border-collapse border border-slate-200 text-xs">
                              <tr className="bg-slate-50 font-bold">
                                <th className="border border-slate-200 p-2 text-left">Role</th>
                                <th className="border border-slate-200 p-2 text-left">Description</th>
                                <th className="border border-slate-200 p-2 text-left">Outcome</th>
                                {renderRowEvidenceHeader("sec_roles_department")}
                              </tr>
                              {selectedAppraisal.formData.accreditationContributions.map((row, i) => (
                                <tr key={i}>
                                  <td className="border border-slate-200 p-2 font-bold text-indigo-700">{row.role}</td>
                                  <td className="border border-slate-200 p-2 text-slate-700">{row.description}</td>
                                  <td className="border border-slate-200 p-2 text-zinc-655">{row.outcome}</td>
                                  {renderRowEvidenceCellReadOnly(row, "sec_roles_department")}
                                </tr>
                              ))}
                            </table>
                        ...
                        {selectedAppraisal.formData?.rdContributions && selectedAppraisal.formData.rdContributions.length > 0 && (
                          <div>
                            <span className="text-xs font-black text-slate-900 block mb-2 uppercase">E) R&D / EDC / SIC / Sports Portfolios</span>
                            <table className="w-full border-collapse border border-slate-200 text-xs">
                              <tr className="bg-slate-50 font-bold">
                                <th className="border border-slate-200 p-2 text-left">Role</th>
                                <th className="border border-slate-200 p-2 text-left">Description</th>
                          ...
                          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2">
                            ...
                            {isSectionVisible("f_resultImprovementHOD") && selectedAppraisal.formData?.resultImprovementHOD && (
                              <div>
                                ...
                                <p className="font-semibold text-slate-900">"{selectedAppraisal.formData.resultImprovementHOD}"</p>
                              </div>
                            )}
                            {isSectionVisible("f_deptAdministrationHOD") && selectedAppraisal.formData?.deptAdministrationHOD && (
                              <div className="mt-2">
                                ...
                                <p className="font-semibold text-slate-900">"{selectedAppraisal.formData.deptAdministrationHOD}"</p>
                              </div>
                            )}
                          </div>
                        )}
                        ...
                        {isSectionVisible("f_otherRolesContribution") && selectedAppraisal.formData?.otherRolesContribution && (
                          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                            <span className="block text-[9px] font-black text-slate-600 uppercase mb-1">
                              {getSectionTitle("f_otherRolesContribution", "Other Role / Contribution")}:
                            </span>
                            <p className="font-semibold text-slate-900">"{selectedAppraisal.formData.otherRolesContribution}"</p>
                          </div>
                        )}

                        {/* Admissions contributed */}
                        <div>
                          <span className="text-xs font-black text-slate-900 block mb-2 uppercase">Admissions Contributed (Minimum 5 Admissions)</span>
                          <table className="w-full border-collapse border border-slate-200 text-xs">
                            <tr className="bg-slate-50 font-bold">
                              <th className="border border-slate-200 p-2">Team No / Area</th>
                              <th className="border border-slate-200 p-2 text-indigo-600enter">Admissions Contributed</th>
                            ...
                            {(selectedAppraisal.formData?.admissionContribution || []).map((row, i) => (
                              <tr key={i}>
                                <td className="border border-slate-200 p-2 font-bold">{row.teamNoArea}</td>
                                <td className="border border-slate-200 p-2 text-indigo-600enter font-black text-indigo-400">{row.countContributed}</td>
                                <td className="border border-slate-200 p-2">{row.teamLeaderName}</td>
                                {renderRowEvidenceCellReadOnly(row, "sec_roles_department")}
                              </tr>
                            ))}
                          </table>
                    ...
                    {isSectionVisible("sec_professional_memberships") && selectedAppraisal.formData?.professionalMembership && selectedAppraisal.formData.professionalMembership.length > 0 && (
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2">
                        ...
                        {getSectionDescription("sec_professional_memberships") && (
                          <p className="text-[10px] text-slate-600 font-semibold uppercase">{getSectionDescription("sec_professional_memberships")}</p>
                        )}
                        <div className="space-y-1">
                          {selectedAppraisal.formData.professionalMembership.map((row, idx) => (
                            <div key={idx} className="bg-white p-2.5 rounded-lg border border-slate-200/60 flex justify-between text-xs items-center">
                              <span className="font-bold text-slate-700">{row.name}</span>
                              <div className="flex items-center gap-2">
                                <span className="font-semibold text-slate-600">{row.type} (No: {row.membershipNo})</span>
                    ...
                    {isSectionVisible("sec_awards_honors") && selectedAppraisal.formData?.awardsHonors && selectedAppraisal.formData.awardsHonors.length > 0 && (
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2">
                        ...
                        {getSectionDescription("sec_awards_honors") && (
                          <p className="text-[10px] text-slate-600 font-semibold uppercase">{getSectionDescription("sec_awards_honors")}</p>
                        )}
                        <table className="w-full border-collapse border border-slate-200 text-xs bg-white">
                          <tr className="bg-slate-50 font-bold">
                            <th className="border border-slate-200 p-2">Award Title</th>
                            <th className="border border-slate-200 p-2">Organization</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">Year</th>
                            <th className="border border-slate-200 p-2 text-indigo-600enter">Level</th>
                            {renderRowEvidenceHeader("sec_awards_honors")}
                          </tr>
                          {selectedAppraisal.formData.awardsHonors.map((row, idx) => (
                            <tr key={idx}>
                              <td className="border border-slate-200 p-2 font-bold text-slate-750">{row.awardName}</td>
                              <td className="border border-slate-200 p-2 font-medium text-zinc-655">{row.organization}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter font-semibold">{row.year}</td>
                              <td className="border border-slate-200 p-2 text-indigo-600enter font-bold text-indigo-750">{row.level}</td>
                    ...
                    {isSectionVisible("sec_library_usage") && (
                      <div className="bg-slate-50 p-4 border border-slate-200 rounded-xl space-y-2">
                        ...
                        {getSectionDescription("sec_library_usage") && (
                          <p className="text-[10px] text-slate-600 font-semibold uppercase">{getSectionDescription("sec_library_usage")}</p>
                        )}
                        {isSectionVisible("f_libraryUsage") && (
                          <div>
                            <span className="block text-[9px] font-black text-slate-600 uppercase mb-0.5">{getSectionTitle("f_libraryUsage", "Use of Library Journals / Books")}</span>
                        ...
                        {isSectionVisible("f_libraryPurpose") && (
                          <div className="text-[10px] font-black text-slate-600 uppercase tracking-widest mt-2">
                            {getSectionTitle("f_libraryPurpose", "Purpose of visit")}: <span className="text-slate-900 font-bold">{selectedAppraisal.formData?.libraryPurpose || "GK"}</span>
                          </div>
                        )}
                      </div>
                    )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {isSectionVisible("f_accomplishAssignment") && (
                            <div className="border border-slate-200 p-3.5 rounded-xl">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider">{getSectionTitle("f_accomplishAssignment", "Conducted assignment in time?")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.accomplishAssignment}</span>
                            </div>
                          )}
                          {isSectionVisible("f_applyLeaveInAdvance") && (
                            <div className="border border-slate-200 p-3.5 rounded-xl">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider">{getSectionTitle("f_applyLeaveInAdvance", "Applied leave in advance?")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.applyLeaveInAdvance}</span>
                            </div>
                          )}
                          {isSectionVisible("f_consumeClLastMonth") && (
                            <div className="border border-slate-200 p-3.5 rounded-xl">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider">{getSectionTitle("f_consumeClLastMonth", "Consume CL last month?")}</span>
                              <span className="text-xs font-bold text-slate-900">{selectedAppraisal.formData?.consumeClLastMonth}</span>
                            </div>
                          )}
                        </div>
                        ...
                        {isSectionVisible("sec_leave_summary") && (
                          <div>
                            ...
                            {getSectionDescription("sec_leave_summary") && (
                              <p className="text-[10px] text-slate-600 font-semibold uppercase mb-2">{getSectionDescription("sec_leave_summary")}</p>
                            )}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-4 rounded-xl border border-slate-200">
                              <div>
                                <span className="text-[10px] font-black text-slate-600 block mb-1 underline uppercase">Leaves Availed</span>
                              ...
                              <div>
                                <span className="text-[10px] font-black text-slate-600 block mb-1 underline uppercase">On Duty (OD) Availed</span>
                                <div className="text-xs font-bold text-slate-900">
                    ...
                    {isSectionVisible("sec_interpersonal_relations") && (
                      <div>
                        ...
                        {getSectionDescription("sec_interpersonal_relations") && (
                          <p className="text-[10px] text-slate-600 font-semibold uppercase mb-3">{getSectionDescription("sec_interpersonal_relations")}</p>
                        )}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {isSectionVisible("f_relationStudents") && (
                            <div className="bg-slate-50 border border-slate-200/50 p-4 rounded-xl">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider">{getSectionTitle("f_relationStudents", "Students")}</span>
                              <span className="text-xs font-black text-indigo-400">{(selectedAppraisal.formData?.relationStudents || {}).rating || "Good"}</span>
                              {(selectedAppraisal.formData?.relationStudents || {}).reason && <p className="text-[10px] text-slate-600 mt-1">Reason: "{(selectedAppraisal.formData?.relationStudents || {}).reason}"</p>}
                            </div>
                          )}
                          {isSectionVisible("f_relationColleagues") && (
                            <div className="bg-slate-50 border border-slate-200/50 p-4 rounded-xl">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider">{getSectionTitle("f_relationColleagues", "Colleagues")}</span>
                              <span className="text-xs font-black text-indigo-400">{(selectedAppraisal.formData?.relationColleagues || {}).rating || "Good"}</span>
                              {(selectedAppraisal.formData?.relationColleagues || {}).reason && <p className="text-[10px] text-slate-600 mt-1">Reason: "{(selectedAppraisal.formData?.relationColleagues || {}).reason}"</p>}
                            </div>
                          )}
                          {isSectionVisible("f_relationSuperiors") && (
                            <div className="bg-slate-50 border border-slate-200/50 p-4 rounded-xl">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider">{getSectionTitle("f_relationSuperiors", "Superiors")}</span>
                              <span className="text-xs font-black text-indigo-400">{(selectedAppraisal.formData?.relationSuperiors || {}).rating || "Good"}</span>
                              {(selectedAppraisal.formData?.relationSuperiors || {}).reason && <p className="text-[10px] text-slate-600 mt-1">Reason: "{(selectedAppraisal.formData?.relationSuperiors || {}).reason}"</p>}
                            </div>
                          )}
                          {isSectionVisible("f_relationDepartment") && (
                            <div className="bg-slate-50 border border-slate-200/50 p-4 rounded-xl">
                              <span className="block text-[9px] font-black text-slate-600 uppercase tracking-wider">{getSectionTitle("f_relationDepartment", "Department")}</span>
                              <span className="text-xs font-black text-indigo-400">{(selectedAppraisal.formData?.relationDepartment || {}).rating || "Good"}</span>
                              {(selectedAppraisal.formData?.relationDepartment || {}).reason && <p className="text-[10px] text-slate-600 mt-1">Reason: "{(selectedAppraisal.formData?.relationDepartment || {}).reason}"</p>}
                            </div>
                          )}
                        </div>
                      </div>
                        ...
                        {isSectionVisible("f_targetsNextSemester") && (
                          <div>
                            ...
                            {getSectionDescription("f_targetsNextSemester") && (
                              <p className="text-[9px] text-slate-600 font-semibold uppercase mb-1">{getSectionDescription("f_targetsNextSemester")}</p>
                            )}
                            <p className="text-xs text-slate-700 font-medium bg-slate-50 p-3 rounded-xl border border-slate-200/60">"{selectedAppraisal.formData?.targetsNextSemester || "N/A"}"</p>
                          </div>
                        )}
                        {isSectionVisible("f_targetsStrategy") && (
                          <div>
                            ...
                            {getSectionDescription("f_targetsStrategy") && (
                              <p className="text-[9px] text-slate-600 font-semibold uppercase mb-1">{getSectionDescription("f_targetsStrategy")}</p>
                            )}
                            <p className="text-xs text-slate-700 font-medium bg-slate-50 p-3 rounded-xl border border-slate-200/60">"{selectedAppraisal.formData?.targetsStrategy || "N/A"}"</p>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    {isSectionVisible("sec_self_analysis") && (
                      <div>
                        ...
                        {getSectionDescription("sec_self_analysis") && (
                          <p className="text-[10px] text-slate-600 font-semibold uppercase mb-2">{getSectionDescription("sec_self_analysis")}</p>
                        )}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {activeDetailsTab === 7 && (
                  <div className="space-y-6 text-xs animate-fadeIn">
                    <div className="border-b border-slate-200/60 pb-2 mb-4">
                      ...
                      <p className="text-[9px] text-slate-600 font-semibold uppercase mt-0.5">Details and attachments configured dynamically by HR.</p>
                    </div>
                        ...
                        return (
                          <div key={field.id} className="bg-slate-50 border border-slate-200/50 p-4 rounded-xl space-y-2">
                            ...
                            {field.type !== "file_only" && entry.value && (
                              <p className="font-bold text-slate-900 glass-input p-3 rounded-lg text-slate-900">{entry.value}</p>
                            )}
                ...
                {selectedAppraisal.autoScore?.breakdown && (
                  <div className="mt-8 border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-xs">
                    ...
                    <table className="w-full border-collapse text-xs">
                      <thead>
                        <tr className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200">
                          ...
                          <th className="p-2.5 text-indigo-600enter text-indigo-700 font-black">Self Analyse Score</th>
                          <th className="p-2.5 text-indigo-600enter text-emerald-700 font-black">HOD Score</th>
                        </tr>
                      </thead>
                      {(() => {
                        ...
                        const rowEls = (rows) => rows.map((r) => {
                          ...
                          return (
                            <tr key={r.id} className="hover:bg-slate-50/50">
                              ...
                              <td className="p-2.5 text-indigo-600enter text-slate-600">{r.value === null ? "—" : String(r.value)}</td>
                              ...
                              <td className="p-2.5 text-indigo-600enter font-black text-emerald-700">{hodVal}</td>
                            </tr>
                          );
                        });
                            ...
                            <tbody className="divide-y divide-zinc-100">
                              {p1.length > 0 && (
                                <tr className="bg-slate-50/70">
                                  <td colSpan={5} className="p-2 text-[10px] font-black text-slate-600 uppercase tracking-wider">Part 1 — Academic & Feedback</td>
                                </tr>
                              )}
                              {rowEls(p1)}
                              {p1.length > 0 && (
                                <tr className="bg-slate-50/70 font-bold">
                                  <td className="p-2 text-right text-[11px] text-slate-700 uppercase" colSpan={2}>Part 1 Total</td>
                                  <td className="p-2 text-indigo-600enter text-slate-800">{p1M}</td>
                                  <td className="p-2 text-indigo-600enter text-indigo-800 font-black">{p1T}</td>
                                  <td className="p-2 text-indigo-600enter text-emerald-800 font-black">{p1HodT}</td>
                                </tr>
                              )}
                              {p2.length > 0 && (
                                <tr className="bg-slate-50/70">
                                  <td colSpan={5} className="p-2 text-[10px] font-black text-slate-600 uppercase tracking-wider">Part 2 — Self & Department Contributions</td>
                                </tr>
                              )}
                              {rowEls(p2)}
                              {p2.length > 0 && (
                                <tr className="bg-slate-50/70 font-bold">
                                  <td className="p-2 text-right text-[11px] text-slate-700 uppercase" colSpan={2}>Part 2 Total</td>
                                  <td className="p-2 text-indigo-600enter text-slate-800">{p2M}</td>
                                  <td className="p-2 text-indigo-600enter text-indigo-800 font-black">{p2T}</td>
                                  <td className="p-2 text-indigo-600enter text-emerald-800 font-black">{p2HodT}</td>
                                </tr>
                              )}
                            </tbody>
                            <tfoot>
                              <tr className="bg-gradient-to-r from-indigo-700 via-emerald-700 to-teal-800 text-white font-black">
                                ...
                                <td className="p-3 text-indigo-600enter text-zinc-200">{gM}</td>
                                <td className="p-3 text-indigo-600enter text-base">{gT}</td>
                                <td className="p-3 text-indigo-600enter text-base text-emerald-200">{gHodT}</td>
                              </tr>
                            </tfoot>
                          </>
                        );
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 h-fit space-y-6">
                ...
                <div>
                  <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                    <Star size={14} className="text-indigo-400" /> Evaluation & Recommendation
                  </h4>
                  <p className="text-[11px] text-slate-600">Provide evaluation grade and recommendation comments for this appraisal request.</p>
                </div>
                ...
                {/* Grade Selection */}
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider mb-1">Recommended Appraisal Grade</label>
                  <select
                    value={evaluationGrade}
                    onChange={(e) => setEvaluationGrade(e.target.value)}
                    className="w-full rounded-xl border border-slate-200 bg-white p-3 text-xs font-bold text-slate-800 focus:outline-none"
                  >
                ...
                {(userRole === "Principal" || userRole === "Admin") && (
                  <div className="border border-slate-200/60 p-4 bg-white rounded-xl space-y-3">
                    <span className="block text-[11px] font-bold text-slate-700 uppercase mb-2">Principal's Remarks Checkboxes</span>
                    <div className="flex flex-col gap-2">
                      <label className="flex items-start gap-2 text-xs font-medium text-slate-700 cursor-pointer">
                        <input
                          ...
                          className="h-4.5 w-4.5 rounded border-slate-300 text-indigo-400 mt-0.5"
                        />
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider mb-1">Detailed Review Comments</label>
                  <textarea
                    value={comments}
                    onChange={(e) => setComments(e.target.value)}
                    rows={4}
                    className="w-full rounded-xl border border-slate-200 bg-white p-3 text-xs font-medium text-slate-800 focus:outline-none"
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm flex flex-wrap items-center justify-between gap-4">
              <div className="flex flex-1 items-center gap-2 max-w-md bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-600">
                <Search size={16} />
                <input
                  type="text"
                  ...
                  className="bg-transparent border-0 text-xs text-slate-800 w-full focus:outline-none focus:ring-0"
                />
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                {userRole !== "HOD" && (
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black text-slate-600 uppercase tracking-wider">Dept:</span>
                    <select
                      value={deptFilter}
                ...
                <table className="w-full border-collapse text-left text-xs">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold">
                      ...
                      <th className="p-4 text-indigo-600enter">Session</th>
                      <th className="p-4 text-indigo-600enter">Status</th>
                      <th className="p-4 text-indigo-600enter">HOD Recommendation</th>
                      <th className="p-4 text-indigo-600enter">Principal Rating</th>
                      <th className="p-4 text-indigo-600enter">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-100">
                    {filteredAppraisals.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="p-8 text-indigo-600enter text-slate-600 font-semibold">
                      ...
                      filteredAppraisals.map((app) => (
                        <tr key={app.id} className="hover:bg-slate-50/40 transition-all">
                          <td className="p-4">
                            <div className="font-bold text-slate-900">{app.facultyName}</div>
                            <div className="text-[10px] text-slate-600">{app.facultyEmail}</div>
                          </td>
                          <td className="p-4">
                            <div className="font-semibold text-slate-700">{app.department}</div>
                            <div className="text-[10px] text-slate-600">{app.designation}</div>
                          </td>
                          <td className="p-4 text-indigo-600enter font-bold text-slate-700">
                            {app.academicYear}
                          </td>
                          <td className="p-4 text-indigo-600enter">
                            <span className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${
                              ...
                              app.status === "Submitted" ? "bg-indigo-600mber-500/10 text-indigo-600mber-700" :
                          ...
                          <td className="p-4 text-indigo-600enter font-bold text-indigo-950">
                            {app.hodReview?.grade ? (
                              <span className="flex items-center justify-center gap-1">
                                <Star size={10} className="fill-amber-400 text-indigo-600mber-400" /> {app.hodReview.grade}
                              </span>
                            ) : (
                              <span className="text-zinc-300">-</span>
                            )}
                          </td>
                          <td className="p-4 text-indigo-600enter font-bold text-emerald-950">
                            {app.principalReview?.grade ? (
                              <span className="flex items-center justify-center gap-1">
                                <Star size={10} className="fill-amber-400 text-indigo-600mber-400" /> {app.principalReview.grade}
                              </span>
                            ) : (
                              <span className="text-zinc-300">-</span>
                            )}
        ...
        {correctionModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-3xl w-full max-w-md border border-slate-200 shadow-xl p-6 space-y-4 animate-scaleUp">
              <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                ...
                <button onClick={() => setCorrectionModalOpen(false)} className="text-slate-600 hover:text-slate-700"><X size={16} /></button>
              </div>

              <div>
                <label className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider mb-1.5">Specify required corrections for faculty</label>
                <textarea
              ...
              <div className="flex justify-end gap-2.5 pt-2">
                <button
                  ...
                  className="px-4 py-2 bg-zinc-100 hover:bg-zinc-200 text-slate-700 rounded-xl text-xs font-bold transition-all cursor-pointer"
                >
                  Cancel
                </button>
                <button
