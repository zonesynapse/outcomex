...
export default function TeacherAppraisal() {
  ...
  const [uploadingExamMap, setUploadingExamMap] = useState({});

  const handleExamFileUpload = async (arrayKey, index, file) => {
    if (!file || !currentUser) return;

    // Strict file size check (< 100 KB)
    if (file.size > 100 * 1024) {
      alert(`File size is ${(file.size / 1024).toFixed(1)} KB. Maximum allowed file size for attachment is 100 KB.`);
      return;
    }

    const uploadKey = `${arrayKey}_${index}`;
    setUploadingExamMap(prev => ({ ...prev, [uploadKey]: true }));
    try {
      const storagePath = userStoragePath(currentUser.uid, "exam_evidence", file.name);
      const downloadUrl = await uploadFile(storagePath, file, file.type);
      handleArrayRowChange(arrayKey, index, "fileUrl", downloadUrl);
      handleArrayRowChange(arrayKey, index, "fileName", file.name);
      alert("Attachment uploaded successfully!");
    } catch (err) {
      console.error("Exam file upload error:", err);
      alert("Failed to upload attachment. Please try again.");
    } finally {
      setUploadingExamMap(prev => ({ ...prev, [uploadKey]: false }));
    }
  };
      ...
      <div className="space-y-6 max-w-6xl mx-auto pb-16">
                ...
                <table className="w-full text-xs text-left">
                  <thead className="bg-slate-50 border-b border-slate-200 text-[10px] uppercase font-bold text-slate-700">
                    <tr>
                      ...
                      <th className="p-3 text-center w-36">Attachment</th>
                      {!isReadOnly && <th className="p-3 text-center w-12">Action</th>}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 font-semibold">
                          ...
                          {row.fileUrl ? (
                            <div className="flex items-center justify-center gap-1">
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="inline-flex items-center gap-1 text-[11px] font-extrabold text-indigo-700 bg-indigo-50 border border-indigo-200 px-2 py-1 rounded-lg hover:bg-indigo-100 transition-all truncate max-w-[110px]"
                                title={row.fileName || "View Attachment"}
                              >
                                <Paperclip className="w-3 h-3 flex-shrink-0" />
                                <span className="truncate">{row.fileName || "Proof"}</span>
                              </a>
                              {!isReadOnly && (
                                <button
                                  type="button"
                                  onClick={() => {
                                    handleArrayRowChange("workshops", idx, "fileUrl", "");
                                    handleArrayRowChange("workshops", idx, "fileName", "");
                                  }}
                            ...
                            <div className="flex flex-col items-center gap-0.5">
                              <label className="inline-flex items-center justify-center gap-1 px-2.5 py-1.5 bg-slate-100 hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 text-slate-700 hover:text-indigo-700 rounded-lg text-[11px] font-bold transition-all cursor-pointer">
                                {uploadingExamMap[`workshops_${idx}`] ? (
                                  <>
                                ...
                                <input
                                  type="file"
                                  className="hidden"
                                  disabled={uploadingExamMap[`workshops_${idx}`]}
                                  onChange={(e) => e.target.files?.[0] && handleExamFileUpload("workshops", idx, e.target.files[0])}
                                />
                              </label>
                              <span className="text-[9px] text-slate-400 font-medium">&lt; 100 KB</span>
                            </div>
                          ) : (
        ...
        {activeTab === 4 && (
          <div className="bg-white rounded-3xl border border-slate-200/80 p-6 md:p-8 space-y-10 shadow-xs">
            ...
            {/* Q20: No. of CL Taken in the last Academic year */}
            <div className="pt-6 border-t border-slate-100">
              <label className="block text-xs font-extrabold text-slate-900 uppercase tracking-wider mb-2">
                20. No. of CL Taken in the last Academic year
              </label>
              <p className="text-[11px] text-slate-500 mb-3">Please enter the number of casual leaves taken (Maximum 12 days).</p>
              <div className="max-w-xs relative">
                <input
                  ...
                  step="1"
                  disabled={isReadOnly}
                  value={formData.consumeBalanceCL ?? ""}
                  onChange={e => {
                    const val = e.target.value;
                    if (val === "") {
                      handleTextChange("consumeBalanceCL", "");
                      return;
                    }
                    const num = parseInt(val, 10);
                    if (!isNaN(num)) {
                      if (num > 12) {
                        handleTextChange("consumeBalanceCL", "12");
                      } else if (num < 0) {
                        handleTextChange("consumeBalanceCL", "0");
                      } else {
                        handleTextChange("consumeBalanceCL", String(num));
                      }
                    }
                  }}
                  placeholder="Enter No. of CLs (0 - 12)"
                  className="w-full p-3 pr-16 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all disabled:opacity-60"
                />
  ...
  return (
    <div className="pt-4">
      <div className="flex items-center justify-between mb-3">
        ...
        {!isReadOnly && (
          <button
            onClick={() => addArrayRow(arrayKey, { class: "", subject: "", appeared: "", passed: "", passPercent: "", subjectAvg: "", fileUrl: "", fileName: "" })}
            className="px-3 py-1 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer"
          >
              ...
              <tr key={idx} className="hover:bg-slate-50/50">
                ...
                <td className="p-2 text-center min-w-[120px]">
                  {row.fileUrl ? (
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {!isReadOnly && (
                        <button
                          type="button"
                          onClick={() => {
                            handleArrayRowChange(arrayKey, idx, "fileUrl", "");
                            handleArrayRowChange(arrayKey, idx, "fileName", "");
                          }}
                          className="p-1 text-slate-400 hover:text-rose-600 rounded-md transition-all cursor-pointer"
                          title="Remove attachment"
                        >
                    ...
                    <div className="flex flex-col items-center gap-0.5">
                      <label className="inline-flex items-center justify-center gap-1 px-2.5 py-1.5 bg-slate-100 hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 text-slate-700 hover:text-indigo-700 rounded-lg text-[11px] font-bold transition-all cursor-pointer">
                        {uploadingExamMap[`${arrayKey}_${idx}`] ? (
                          <>
                            <Loader2 className="w-3 h-3 animate-spin text-indigo-600" />
                            <span>Uploading...</span>
                          </>
                        ...
                        <input
                          type="file"
                          className="hidden"
                          disabled={uploadingExamMap[`${arrayKey}_${idx}`]}
                          onChange={(e) => e.target.files?.[0] && handleExamFileUpload(arrayKey, idx, e.target.files[0])}
                        />
                      </label>
                      <span className="text-[9px] text-slate-400 font-medium">&lt; 100 KB</span>
                    </div>
