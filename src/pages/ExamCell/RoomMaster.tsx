...
export const DEFAULT_CKCET_ROOMS: Room[] = [];
...
export const RoomMaster: React.FC<RoomMasterProps> = ({ rooms, onUpdateRooms }) => {
  const [selectedRoom, setSelectedRoom] = useState<Room | null>(rooms && rooms.length > 0 ? rooms[0] : null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-5 border border-zinc-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2 text-[#120c7a] font-bold text-xs tracking-wider uppercase">
          ...
          <h2 className="text-xl font-black text-zinc-900 mt-1">
            Exam Hall Seating Matrix & Registry
          </h2>
        ...
        <div className="flex items-center space-x-3">
          <div className="flex items-center space-x-2 bg-zinc-100 p-1.5 rounded-xl text-xs">
            <span className="text-zinc-600 pl-2 font-bold">Filter Block:</span>
            <select
              id="room-block-filter"
              value={filterBlock}
              onChange={(e) => setFilterBlock(e.target.value)}
              aria-label="Filter rooms by building block"
              className="bg-white px-3 py-1.5 rounded-lg font-bold text-zinc-800 border border-zinc-200 focus:outline-none focus:ring-2 focus:ring-[#120c7a]/20 cursor-pointer"
            >
              {blocks.map((b) => (
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-2.5 bg-[#120c7a] hover:bg-[#0f0a66] text-white rounded-xl text-xs font-bold shadow-md transition-all cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7">
          {selectedRoom ? (
            ...
            <div className="bg-white rounded-2xl p-12 text-center border border-zinc-200 shadow-sm flex flex-col items-center justify-center space-y-4">
              <div className="w-16 h-16 rounded-2xl bg-blue-50 text-[#120c7a] border border-blue-200 flex items-center justify-center">
                <Building2 size={32} />
              </div>
              <h3 className="text-xl font-black text-zinc-900">No Examination Halls Registered</h3>
              <p className="text-xs text-zinc-500 max-w-md font-medium">
                No rooms found in Firestore database. Click below to add a new hall and configure its seating matrix!
              </p>
              <button
                onClick={handleOpenAdd}
                className="px-5 py-2.5 bg-[#120c7a] hover:bg-[#0f0a66] text-white font-extrabold text-xs rounded-xl shadow-md transition-all cursor-pointer inline-flex items-center gap-2"
              >
                <Plus size={16} />
                <span>Create New Exam Hall / Room</span>
              </button>
            </div>
          )}
        </div>
